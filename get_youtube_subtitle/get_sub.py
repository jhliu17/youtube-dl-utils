...
class Sub_getter(object):
    ...
    def req_api(self, v_url):
        have_sub = requests.get(
            self.api_url+v_url[-11:]+'?'+'api-key='+self.token).json()
            ...
            for i in sub_list:
                ...
                if self.config['multilanguage']:
                    if self.config['which_language_to_zh'] in i['language']:
                        print('Find （'+sub_title+'） 【' +
                              i['language']+' and zh-Hans】 subtitle!')
                        self.get_sub(i['caption_content_url'],
                                     sub_title, **self.config)
                        find = True
                        break
                # 单语言字幕
                else:
    ...
    def download_list(self, tasks):
        ...
        print('Download complete,Success get:【', self.complete,
              '】subtitles. Failure:【', len(tasks)-self.complete, '】')
